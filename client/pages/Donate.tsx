...
function useAuthToken() {
  ...
  useEffect(() => { setToken(localStorage.getItem("fb_token")); }, []);
  return token;
}
  ...
  return (
    <div className="mt-4">
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <span className="text-sm text-muted-foreground">Payments are processed in INR via Stripe.</span>
      </div>
    </div>
  );
}
...
function MoneyDonationINR() {
  ...
  async function initialize() {
    ...
    const res = await fetch("/api/payments/create-intent", {
      ...
      body: JSON.stringify({ amount: Math.round(amountInr * 100), currency: "inr", userId }),
    });
    const data = await res.json();
    ...
    <div className="rounded-2xl glass p-6 shadow-lg ring-1 ring-primary/5">
      ...
      {!stripePk && (
        <div className="mt-4 rounded-md border bg-muted p-3 text-sm text-muted-foreground">
          Stripe not configured. Set STRIPE_SECRET_KEY and VITE_STRIPE_PUBLISHABLE_KEY.
        </div>
      )}
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div>

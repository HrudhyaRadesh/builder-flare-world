...
export default function NotificationsPage() {
  ...
  async function updateStatus(
    ...
    if (res.ok)
      setDonations((prev) =>
        prev.map((r) => (r.id === donationId ? { ...r, status } : r)),
      );
  }
    ...
    <Layout>
      <section className="container py-16">
        ...
        <p className="mt-2 text-muted-foreground">
          NGOs and admins can see pickup requests and updates. Open map links
          for directions.
        </p>
          ...
          {items.map((n) => {
            ...
            const mapUrl =
              d?.donorLat && d?.donorLng
                ? `https://www.google.com/maps?q=${d.donorLat},${d.donorLng}`
                : null;
            return (
              <div
                key={n.id}
                className={`rounded-2xl glass p-5 shadow-sm ring-1 ring-primary/5 ${n.read ? "opacity-75" : ""}`}
              >
                <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="font-medium">{n.message}</p>
                    <p className="text-xs text-muted-foreground">
                      {new Date(n.createdAt).toLocaleString()} · Donation #
                      {n.donationId} · Status: {d?.status ?? "-"}
                    </p>
                    {mapUrl && (
                      <a
                  ...
                  <div className="flex gap-2">
                    ...
                    {d && d.status !== "distributed" && (
                      <button
                        onClick={() => updateStatus(d.id, "distributed")}

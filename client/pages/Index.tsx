  ...
  return (
    <div className="rounded-2xl glass p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Specify food category, quantity, expiry, and pickup location. Receiver
    ...
    <div className="mt-4">
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <span className="text-sm text-muted-foreground">
          Login recommended to attribute rewards
        </span>
      </div>
    </div>
  );
...
function MoneyDonation() {
  ...
  return (
    <div className="rounded-2xl glass p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Support logistics and storage. Real payments via Stripe.
      </p>
      {!stripePk && (
        <div className="mt-4 rounded-md border bg-muted p-3 text-sm text-muted-foreground">
          Stripe is not yet configured. Set STRIPE_SECRET_KEY (server) and
          VITE_STRIPE_PUBLISHABLE_KEY (client). You can deploy with Netlify or
          Vercel and set env vars there. To use managed hosting, connect via
          Open MCP popover.
        </div>
      )}
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
...
function Leaderboard() {
  const [rows, setRows] = useState<
    { userId: string; name: string; totalQuantity: number; donations: number }[]
  >([]);
  useEffect(() => {
        ...
        {rows.slice(0, 5).map((r, i) => (
          <li key={r.userId} className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="inline-flex size-8 items-center justify-center rounded-full bg-accent font-bold">
                {i + 1}
              </span>
              <div>
    ...
    <div className="rounded-2xl glass p-6 shadow-sm">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-4">
        <div className="rounded-md border p-4">
          ...
          <p className="mt-1 text-2xl font-bold">{stats.totalDonations}</p>
        </div>
        <div className="rounded-md border p-4">
          <p className="text-xs text-muted-foreground">Total Meals</p>
          <p className="mt-1 text-2xl font-bold">{stats.totalQuantity}</p>
        </div>
        <div className="rounded-md border p-4">
          <p className="text-xs text-muted-foreground">Distributed</p>
          <p className="mt-1 text-2xl font-bold">{stats.distributedCount}</p>
        </div>
        <div className="rounded-md border p-4">
    ...
    <Layout>
      <section className="container py-16 relative overflow-hidden">
        ...
        <div className="grid items-center gap-10 md:grid-cols-2">
          <div>
            <h1 className="text-5xl/tight font-extrabold tracking-tight gradient-text">
              End Hunger. Share Surplus. Power Communities.
            </h1>
            <p className="mt-4 text-lg text-muted-foreground">
              Plateful connects donors, NGOs, and the hungry to reduce food
              waste and improve food security. Donate meals or funds and climb
              the leaderboard with impact badges.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a
                href="/donate"
                className="rounded-md px-5 py-3 font-semibold btn-gradient"
              >
                Donate Now
              </a>
              <a href="/login" className="rounded-md border px-5 py-3">
                Sign in
              </a>
            </div>
          </div>
        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="rounded-2xl glass p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Sign in to track your rank and earn badges as you donate more
              food.
            </p>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Next badge at 10 meals donated.
            </p>
          </div>
        </div>
      </section>

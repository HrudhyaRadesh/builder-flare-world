    ...
    <Layout>
      <section className="container py-16">
        <div className="grid gap-10 md:grid-cols-2 items-center">
          <div>
            <h1 className="text-4xl/tight font-extrabold tracking-tight">
              Join Plateful
            </h1>
            <p className="mt-3 text-muted-foreground">
              Create an account or sign in to donate surplus food or funds.
              Roles: Users (donors), NGOs (distributors), Admin (oversight).
            </p>
          ...
          >
            <div className="flex gap-2 mb-6">
              <button
                ...
                className={`flex-1 rounded-md border px-4 py-2 ${mode === "login" ? "bg-primary text-primary-foreground" : "bg-muted"}`}
              >
                Login
              </button>
              <button
                ...
                className={`flex-1 rounded-md border px-4 py-2 ${mode === "register" ? "bg-primary text-primary-foreground" : "bg-muted"}`}
              >
                Register
              </button>
            </div>
                ...
                <div className="flex gap-2">
                  {(["user", "ngo", "admin"] as const).map((r) => (
                    <button
                      ...
                      type="button"
                      onClick={() => setRole(r)}
                      className={`flex-1 rounded-md border px-3 py-2 capitalize ${role === r ? "bg-secondary" : "bg-muted"}`}
                    >
                      {r}
                    </button>
                  ))}
